...
setwd("~/OneDrive - University of Vermont/centaurea/data/")

### Data import and merging ###
P <- read.csv("Parents_greenhouseExpt/Data_Greenhouse_Flowcyt_Corrected_SRK.csv", header=T)
gen <- read.table("Genetics/GeneticData_180320_corrected.txt", header=T)
merged <- merge(P,gen, by.x="fam2", by.y="Ind")
...
# ggsave("../manuscripts/AdmixedTraits/trait_plots.pdf", traits)
...
# ggsave("../manuscripts/AdmixedTraits/trait_plots_pop.pdf", width=14,height=4, traitsp)

# make histograms of trait values by ancestry class accounting for pop effects
...
histp <- marrangeGrob(hist_plot_list, nrow=1, ncol=6, top=NULL)
# ggsave("../manuscripts/AdmixedTraits/hist_plots.pdf", hist)

# Without pop effects
multpanel <- marrangeGrob(c(traits,hist), nrow=2,ncol=1, top=NULL, padding = unit(10, "line"))
ggsave("../manuscripts/AdmixedTraits/traits_hist_fig.pdf", multpanel, width=14,height=8)

# With pop effects
multpanelp <- marrangeGrob(c(traitsp,histp), nrow=2,ncol=1, top=NULL, padding = unit(10, "line"))
ggsave("../manuscripts/AdmixedTraits/FigureS1traits_hist_fig_pop.pdf", multpanelp, width=14,height=8)
...
# 
# for(j in 1:6){
#   #dat_hyb$trait = scale(dat_hyb[j])
#   #max = 1.3*max(dat_hyb$trait)
#   #min = 0.9*min(dat_hyb$trait)
#   max = 1.3*max(dat_hyb[j])
#   min = 0.9*min(dat_hyb[j])
#     
# p <- ggplot(dat_hyb, aes(x=dat_hyb[j], color=class)) + stat_ecdf(size=0.5) + theme(legend.position = "none") + 
#       ggtitle(names(dat_hyb[j])) + xlab("") + ylab("ECDF") + theme(axis.title.x=element_text(size=0), axis.title.y=element_text(size=10)) + 
#       scale_color_manual(values=c("blue","red","purple"))
# ecdf_plot_list[[j]] = p
# }
# 
# ecdf <- marrangeGrob(ecdf_plot_list, nrow=2, ncol=3, top=NULL)
# ggsave("../manuscripts/AdmixedTraits/ecdf_plots.pdf", ecdf)
